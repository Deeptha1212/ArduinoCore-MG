...
#define CONTROL_REG(gpt_num)       GPTIMER_REG(gpt_num) -> CTRL 
#define CLOCK_CNTRL(gpt_num)       GPTIMER_REG(gpt_num) -> CLOCK_CTRL
#define COUNTER_VAL(gpt_num)       GPTIMER_REG(gpt_num) -> COUNT
#define REPEATD_COUNT(gpt_num)     GPTIMER_REG(gpt_num) -> RPTD_COUNT
#define DUTY_CYCL(gpt_num)         GPTIMER_REG(gpt_num) -> DUTY_CYCLE
#define PERIOD_REG(gpt_num)        GPTIMER_REG(gpt_num)->  PERIOD
#define CAPTR_VAL(gpt_num)         GPTIMER_REG(gpt_num) -> CAPTURE_INP
...
uint32_t GPT_Init(GPTIMER_Config_t *gptimer)
{
    ...
    PINMUX_EnableGPTimer(gptimer->gpt_num);
