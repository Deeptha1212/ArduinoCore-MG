...
void wr_uart_tx_dma_threshold (uint8_t uart_number, int mode)
{
	uint16_t temp = uart_instance[uart_number]->CTRL;
	temp &= 0x3c7f; // Mask to preserve other fields except dma_tx_threshold
	uart_instance[uart_number]->CTRL = temp | UART_TX_THRESH(mode);
}

void wr_uart_rx_dma_threshold (uint8_t uart_number, int mode)
{
	uint16_t temp = uart_instance[uart_number]->CTRL;
	temp &= 0x23FF; // Mask to preserve other fields except dma_rx_threshold
	uart_instance[uart_number]->CTRL = temp | UART_RX_THRESH(mode);
}

void uart_pullup_en(UART_Config_t *uart_config)
{
  uint16_t temp = uart_instance[uart_config->uart_num]->CTRL;
  temp &= 0x1fff;
  uart_instance[uart_config->uart_num]->CTRL = temp | UART_PULLUP_EN(uart_config->pullup);
}
...
uint8_t UART_Flush(UART_Config_t *uart_config)
{
	...
	wr_uart_rx_dma_threshold(uart_config->uart_num,0x1);
	...
	wr_uart_rx_dma_threshold(uart_config->uart_num,uart_config->transfer_mode);
	...
	switch (uart_config->receive_mode)
	{
		case DATA_SIZE_8:
			...
			*(uint8_t*)(rx_bufs->uart_data) = uart_instance[uart_config->uart_num]->RX_REG.data_8;
			log_debug("\n0fully read a character from software buffer of instance %d",uart_config->uart_num);	
			break;
		case DATA_SIZE_16:

			while ((uart_instance[uart_config->uart_num]->STATUS_REG & STS_OUTP_READY) ==0 );
...
uint8_t UART_Read_String(UART_Config_t *uart_config , uint8_t * string_data)
{
	log_info("\nReading the string from software buffer of instance %d",uart_config->uart_num);	
	uint32_t i = 0;
	uint8_t temp = 0;
	wr_uart_rx_dma_threshold(uart_config->uart_num,0x01);
	while(1)
	{
		...
		if ((uart_instance[uart_config->uart_num]->STATUS_REG & STS_RX_NOT_EMPTY)==0)
		{
		if(i!=0)
			{
				log_debug("\nData has been stored 0fully");
				string_data[i++] = 0x00;
				wr_uart_rx_dma_threshold(uart_config->uart_num,uart_config->transfer_mode);
				return 0;
			}
